...
│   └── src/
│       ├── server.ts
│       ├── config.ts      # env var loading (Anthropic key, JWT secret)
│       ├── middleware/    # requireAuth.ts — verifies the JWT bearer token
│       ├── services/      # claudeClient.ts (dialogue), userStore.ts (flat-file auth store)
...
- **TTS:** `expo-speech` — wraps the OS's built-in text-to-speech. Free, no API key, and
  ...
  client.
- **Reasoning/dialogue:** unchanged — Claude (`@anthropic-ai/sdk`), implemented in
  `backend/src/services/claudeClient.ts`, with separate English/Arabic system prompts.
- **Flow:** app listens via `Voice.start(locale)` → on-device transcript →
  `POST /api/voice/text { text, language }` → Claude reply (text only, no backend audio
...
- Required env var (see `backend/.env.example`): `ANTHROPIC_API_KEY` only.
...
- **Flow:** push this repo to GitHub → Render **New + → Blueprint** picks up
  `render.yaml` → set `ANTHROPIC_API_KEY` in the Render dashboard (`JWT_SECRET` is
...
- Voice loop (`app/src/screens/VoiceScreen.tsx` + `backend/src/routes/voice.ts`): fully
  ...
  above), just needs `ANTHROPIC_API_KEY` and, for the mic specifically, a custom dev build
...
2. `cd backend && npm install`, copy `.env.example` to `.env` and fill in
   `ANTHROPIC_API_KEY` and `JWT_SECRET`, then `npm run dev`. For a public URL (needed for
